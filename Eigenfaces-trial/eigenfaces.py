...
class Eigenfaces(object):                                                       # *** COMMENTS ***
    ...
    def evaluate_celebrities(self, celebrity_dir='.'):
        ...
        for img_name in os.listdir(celebrity_dir):                              # go through all the celebrity images in the folder
            ...
            #print top5_ids 
            flag=1                                        # open the results file for writing
            for top_id in top5_ids:
                ...
                shutil.copyfile(path_to_img,                                    # copy the top face from source
                        os.path.join(result_dir,'s'+str(face_id)+'_'+str(top_id) + '.pgm'))         # to destination

                f.write('id: %3d, distance: %.6f\n' % (face_id, norms[top_id]))     # write the id and its score to the results file
                if(norms[top_id]<2500):
                    flag=0
                    accvalue=(5000-norms[top_id])/5000.0*100
                    f.write('Access permitted. Similarity is %.2f %%\n' % (accvalue))
            
            if(flag==1):
                f.write('Access Denied. Please contact administrator!')

            f.close()                                                           # close the results file
        print '> Evaluating player matches ended'
